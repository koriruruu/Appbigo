...
def generar_datos(n, seed=42):
    ...
    # ~2/7 de probabilidad para fin de semana
    es_fin_de_semana = rng.choice([True, False], size=n, p=[2 / 7, 5 / 7])
...
def alarma_logica_loop(
    ...
    for temp, hum, finde in zip(temperaturas, humedades, es_fin_de_semana):
        resultados.append(
            temp > temp_umbral and hum < hum_umbral and not finde
        )
...
def alarma_logica_vectorizada(
    ...
    # En NumPy usamos & para AND y ~ para NOT elemento a elemento
    return (
...
# ---------------------------------------------------------------------------
with tab1:
    st.subheader("Alarma por regla lógica (3 Proposiciones)")
    st.write(
        "La alarma dispara evaluando tres condiciones con un **AND** y un **NOT** lógico: "
    ...
    temps, hums, findes = generar_datos(n)

    with col_cfg:
        alarmas = alarma_logica_vectorizada(
            temps, hums, findes, temp_umbral, hum_umbral
        )
...
with tab2:
    ...
    st.write(
        "El detector recorre `n` lecturas ordenadamente: es un algoritmo **O(n)**."
    )
    ...
    ax2.plot(
        n_valores, n_valores, label="O(n) — lineal (nuestro detector de 3 provs)"
    )
    ax2.plot(
        n_valores,
...
with tab3:
    ...
    n_bench = st.select_slider(
        ...
        value=1_000_000,  # Predeterminado en 1 millón
    )
    ...
    if st.button("▶️ Ejecutar benchmark", type="primary"):
        temps_b, hums_b, findes_b = generar_datos(n_bench)
        ...
        for _ in range(repeticiones_loop):
            alarma_logica_loop(
                temps_b, hums_b, findes_b, temp_umbral_b, hum_umbral_b
            )
        ...
        if t_vec > 0:
            ...
            col3.metric(
                "NumPy es más rápido por", "demasiado rápido para medir"
            )
